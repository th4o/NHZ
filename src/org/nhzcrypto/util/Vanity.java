...
public class Vanity {
    ...
    static public String generateAccount(final String secretPhrase){
        try {
            MessageDigest sha256 = MessageDigest.getInstance("SHA-256");
            final byte[] bytes = secretPhrase.getBytes();
            final byte[] publicKey = new byte[32];
            
            sha256.update(bytes);
            keygen(publicKey, null, sha256.digest());
            sha256.update(publicKey);
            
            byte[] publicKeyHash = sha256.digest();
            ...
            String account = bigInteger.toString();
            return account;
        } catch (Exception e) {
            e.printStackTrace();
        }
...
//    private static final byte start = 33; // use all ascii chars and symbols from '!' (33) to '~' (126)
//    private static final byte end = 127;

//    public static void main(final String[] args) {
//
//        switch (args.length) {
//            case 0:
//                final String secretPrefix = makeRandomSecretPhrase();
//                System.out.println("Using a randomly generated secret prefix: " + secretPrefix);
//                generatePasswords(vanityFilter, secretPrefix);
//                break;
//            case 1:
//                printWarning();
//                System.out.println("Using a user-supplied secret prefix: " + args[0]);
//                generatePasswords(vanityFilter, args[0]);
//                break;
//            case 2:
//                printWarning();
//                System.out.println("Using a user-supplied secret prefix: " + args[0]);
//                System.out.println("Looking for the shortest account numbers beginning with " + args[1]);
//                generatePasswords(new PrefixFilter(args[1]), args[0]);
//                break;
//            default:
//                System.out.println("Usage: java -jar Vanity.jar [SECRET_PREFIX] [ACCOUNT_PREFIX]");
//                System.exit(1);
//        }
//        System.exit(0);
//
...
//        System.out.println("WARNING: For a given secret prefix, the passwords this program generates\n" +
...
//    private static String makeRandomSecretPhrase() {
//        Random random = new SecureRandom();
//        byte[] bytes = new byte[16];
//        for (int i = 0; i < bytes.length; i++) {
//            bytes[i] = (byte)(random.nextInt(end - start + 1) + start);
//        }
//        return new String(bytes, Charset.forName("UTF-8"));
//    }

//    private static void generatePasswords(final Filter filter, final String secretPrefix) {
//
//        System.out.println("Generating vanity accounts...");
//        final int threadCount = Math.min(Runtime.getRuntime().availableProcessors(), end - start);
//        final AtomicLong totalAttempts = new AtomicLong();
//        final long startTime = System.currentTimeMillis();
//        Thread[] threads = new Thread[threadCount];
//        for (int i = 0; i < threadCount; i++) {
//            final byte threadStartByte = (byte)(start + i);
//            final byte step = (byte)threadCount;
//            Thread thread = new Thread(new Runnable() {
//                @Override
//                public void run() {
//                    try {
//                        long threadAttempts = 0;
//                        MessageDigest sha256 = MessageDigest.getInstance("SHA-256");
//                        final byte[] prefixBytes = secretPrefix.getBytes();
//                        final byte[] bytes = Arrays.copyOf(prefixBytes, prefixBytes.length + 10);
//                        final byte[] publicKey = new byte[32];
//                        final int from = prefixBytes.length;
//                        int current = from;
//                        while (current < bytes.length) {
//                            for (bytes[current] = threadStartByte; bytes[current] <= end - step; bytes[current] += step) {
//                                sha256.update(bytes, 0, current + 1);
//                                keygen(publicKey, null, sha256.digest());
//                                sha256.update(publicKey);
//                                byte[] publicKeyHash = sha256.digest();
//                                BigInteger bigInteger = new BigInteger(1, new byte[] {publicKeyHash[7], publicKeyHash[6], publicKeyHash[5], publicKeyHash[4], publicKeyHash[3], publicKeyHash[2], publicKeyHash[1], publicKeyHash[0]});
//                                String account = bigInteger.toString();
//                                threadAttempts += 1;
//                                if (threadAttempts % 100000 == 0) {
//                                    long total = totalAttempts.addAndGet(threadAttempts);
//                                    threadAttempts = 0;
//                                    if (total % 1000000 == 0) {
//                                        long elapsedTime = System.currentTimeMillis() - startTime;
//                                        System.out.println(total + " passwords tried in " + elapsedTime/1000 + " seconds, " + 1000 * total/elapsedTime + " per second");
//                                    }
//                                }
//                                if (filter.isVanity(account)) {
//                                    System.out.println(new String(bytes, 0, current + 1, Charset.forName("UTF-8")) + " gives account number " + account);
//                                }
//                            }
//                            int previous = current;
//                            boolean allDone = true;
//                            while (previous >= from) {
//                                if (bytes[previous] + 1 < end && previous != current) {
//                                    bytes[previous] += 1;
//                                    allDone = false;
//                                    break;
//                                } else {
//                                    bytes[previous] = start;
//                                    previous -= 1;
//                                }
//                            }
//                            if (allDone) {
...
//        public boolean isVanity(String account);
//    }
//
//    private static final Filter vanityFilter = new Filter() {
//
//        private final Pattern sequences = Pattern.compile("01234567|12345678|23456789|34567890");
//
//        public boolean isVanity(String account) {
//            if (sequences.matcher(account).find()) {
//                return true;
//            }
//            int repetitions = 0;
//            for (int i = 1; i < account.length(); i++) {
//                if (account.charAt(i - 1) == account.charAt(i)) {
//                    repetitions += 1;
//                }
//            }
//            return repetitions > account.length() / 2;
//        }
//    };

//    private static class PrefixFilter implements Filter {
//
//        private final String accountPrefix;
//        private final AtomicInteger maxLength;
//
//        public PrefixFilter(String accountPrefix) {
//            this.accountPrefix = accountPrefix;
//            this.maxLength = new AtomicInteger(Integer.MAX_VALUE);
//        }
//
//        public boolean isVanity(String account) {
//            int limit;
//            while ((limit = maxLength.get()) > account.length()) {
//                if (! account.startsWith(accountPrefix)) {
//                    return false;
//                }
//                if (maxLength.compareAndSet(limit, account.length())) {
//                    return true;
//                }
//            }
//            return false;
