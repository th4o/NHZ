...
import org.nhzcrypto.accounts.AccountsInfoHelper;
...
import org.nhzcrypto.util.Vanity;
            ...
            public void onResponse(boolean success, Account account, String info) {
                if ( success ){
                    if ( account.mBalance >= mFee ){
                        // sign transaction
                        NHZTransaction transaction = 
                                NHZApi.makeAliasTransaction(mSecret, mAlias, mUri, mFee, (short) mDeadline);
                        
                        // broadcast transaction
                        String addr = NodesManager.sharedInstance().getCurrentNode().getIP();
                        new NHZApi().broadcastTransaction(addr, transaction, new NHZApi.ResponseListener() {
                            @Override
